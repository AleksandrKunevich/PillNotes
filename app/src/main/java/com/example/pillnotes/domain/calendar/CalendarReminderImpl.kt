...
class CalendarReminderImpl @Inject constructor(private val context: Context) : CalendarReminder {
    ...
    override fun addEventCalendar(noteTask: NoteTask): Boolean {
        if (isCalendarPermission()) {
            val uid = noteTask.uid.toString()
            val eventTitle = noteTask.title
            val eventDescription = noteTask.task
            val eventDay =
                noteTask.time.substring(Constants.DAY_START_INDEX, Constants.DAY_END_INDEX)
            val eventTime =
    ...
    override fun deleteEvent(noteTask: NoteTask): Boolean {
        if (isCalendarPermission()) {
    ...
    private fun isCalendarPermission(): Boolean {
        if ((ContextCompat.checkSelfPermission(
            ...
            Toast.makeText(
                context,
                context.getString(R.string.no_calendar_perm),
                Toast.LENGTH_SHORT
            )
                .show()
        } else {
